...
function ListOfStudents() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                let response = await fetch('http://localhost:8000/admin/students', {
                    method: 'GET',
    ...
    const handleClick = async (event, students) => {
        try {
            let response = await fetch('http://localhost:8000/admin/student/' + students._id, {
                method: 'DELETE',
            ...
            if (response.status === 200) {
                console.log(result);
                window.alert('this student deleted)');
            }
