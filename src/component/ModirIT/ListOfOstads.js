...
function ListOfOstads() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                let response = await fetch('http://localhost:8000/admin/Professors', {
                    method: 'GET',
    ...
    const handleClick = async (event, students) => {
        try {
            let response = await fetch('http://localhost:8000/admin/Professor/' + students._id, {
                method: 'DELETE',
